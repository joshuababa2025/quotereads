...
-- RLS policies
CREATE POLICY IF NOT EXISTS "Anyone can view user rankings"
ON public.user_rankings FOR SELECT
USING (true);

CREATE POLICY IF NOT EXISTS "Users can insert their own ranking"
ON public.user_rankings FOR INSERT
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own ranking"
...
-- RLS policies: public can read active packages, authenticated can manage (optional)
CREATE POLICY IF NOT EXISTS "Public can read active giveaway packages"
ON public.giveaway_packages FOR SELECT
USING (is_active = true);

-- Optional: allow authenticated users to manage packages (comment out if not desired)
CREATE POLICY IF NOT EXISTS "Authenticated can insert giveaway packages"
ON public.giveaway_packages FOR INSERT
TO authenticated
WITH CHECK (true);

CREATE POLICY IF NOT EXISTS "Authenticated can update giveaway packages"
...
BEFORE UPDATE ON public.ggiveaway_packages
